...
> ```
> ## 測試 ##
> chmod a+x  /usr/local/sbin/getmesg
> /usr/local/sbin/getmesg
> ```
...
> 解答
> ```
> vim /usr/local/sbin/chsel
> ##  chsel  的內容 ##
> ```
![image](https://github.com/vbkservices/mybookword/assets/97799165/760e8054-5e36-4f30-8f21-f69380c3cdd3)
>``` ## 測試 ##
> chmod a+x  /usr/local/sbin/chsel
> /usr/local/sbin/chsel 0
> ```
![image](https://github.com/vbkservices/mybookword/assets/97799165/e0e8def0-84aa-4731-857e-842af77953a8)
> ```
> /usr/local/sbin/chsel 1
> ```
![image](https://github.com/vbkservices/mybookword/assets/97799165/cc342037-bf30-4c4b-954a-20ff65420924)
...
> 解答
> ```
> vim /usr/local/bin/chupper
> ##  chupper  的內容 ##
> cat /etc/hosts | tr "[a-z]" "[A-Z]"
> ```
> ```
> ## 測試 ##
> chmod a+x /usr/local/bin/chupper
> /usr/local/bin/chupper
> ```
...
> 解答
> ```
> vim /usr/local/bin/listcmd.sh
> ##  listcmd.sh  的內容 ##
> ```
  ![image](https://github.com/vbkservices/mybookword/assets/97799165/7b74a81c-6708-4cf6-9219-038ee4bcf258)

> ```
> ## 測試 ##
> chmod a+x /usr/local/bin/listcmd.sh
> /usr/local/bin/listcmd.sh
> echo $? # 2
> /usr/local/bin/listcmd.sh passwd
> echo $? # 0
> /usr/local/bin/listcmd.sh shadow
> echo $? # 0
> /usr/local/bin/listcmd.sh nonfile
> echo $? # 10
> ```
...
> 解答
> ```
> vim /usr/local/bin/myheha
> ##  myheha  的內容 ##
> ```
  ![image](https://github.com/vbkservices/mybookword/assets/97799165/00d82bbf-dbc9-4dec-ba24-b162dc886659)
> ```
> ## 測試 ##
> chmod a+x /usr/local/bin/myheha
> /usr/local/bin/myheha hehe
> /usr/local/bin/myheha haha
> /usr/local/bin/myheha nono
> ```
...
> 解答
> ```
> vim /usr/local/bin/yourbday.sh
> ##  yourbday.sh  的內容 ##
> ```
  ![image](https://github.com/vbkservices/mybookword/assets/97799165/b2ff8d19-0ce5-49ce-8fc0-2ac58d281fe3)
> ```
> ## 測試 ##
> chmod a+x /usr/local/bin/yourbday.sh
> /usr/local/bin/yourbday.sh
> /usr/local/bin/yourbday.sh 2001-03-53
> /usr/local/bin/yourbday.sh 2001-03-21
> ```
...
> 解答
> ```
> vim /usr/local/sbin/examcheck
> ##  examcheck  要修改的內容 ##
> #! /bin/basH >>>> #! /bin/bash
> if ["${ans}" == "ok"];then >>>> if [ "${ans}" == "ok" ];then
> elseif ["${ans}" == "false"];then >>>> elif [ "${ans}" == "false" ];then
> echo -e "\nUsage......false\n >>>> echo -e "\nUsage......false\n" #最後一個加雙引號 "
> ##  examcheck  正確的內容 ##
> ```
> ![image](https://github.com/vbkservices/mybookword/assets/97799165/8873f44f-b497-43e8-a81d-a9d3076567ed)
> ```
> ## 測試 ##
> examcheck ok
> examcheck false
